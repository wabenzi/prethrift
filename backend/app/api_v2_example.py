...
async def enhanced_search(
    request: SearchRequest, search_engine: HybridSearchEngine = Depends(get_search_engine)
) -> SearchResponse:
    """
    ...
    try:
        ...
        filters_dict = {}
        if request.filters:
            filters_dict = {k: v for k, v in request.filters.dict().items() if v is not None}
...
async def get_garment(
    garment_id: int, session: Session = Depends(get_db_session)
) -> GarmentResponse:
...
async def find_similar_garments(
    ...
    try:
        results = search_engine.similar_garments(garment_id=garment_id, limit=limit)
...
async def extract_garment_properties(
    ...
    try:
        success = ontology_service.extract_properties(
            garment=garment, session=session, force_reextract=force_reextract
        )
        ...
        if success:
            return {
                ...
                "extracted_at": garment.properties_extracted_at.isoformat()
                if garment.properties_extracted_at
                else None,
            }
        else:
...
async def get_filter_options(session: Session = Depends(get_db_session)) -> Dict[str, List[str]]:
    ...
    try:
        ...
        # Categories
        categories = (
            session.query(Garment.category).distinct().filter(Garment.category.isnot(None)).all()
        )
        ...
        # Materials
        materials = (
            session.query(Garment.material).distinct().filter(Garment.material.isnot(None)).all()
        )
        ...
        # Designer tiers
        tiers = (
            session.query(Garment.designer_tier)
            .distinct()
            .filter(Garment.designer_tier.isnot(None))
            .all()
        )
...
async def get_system_stats(session: Session = Depends(get_db_session)) -> Dict[str, Any]:
    ...
    try:
        ...
        with_clip_embeddings = (
            session.query(Garment).filter(Garment.image_embedding_vec.isnot(None)).count()
        )
        with_openai_embeddings = (
            session.query(Garment).filter(Garment.openai_text_embedding_vec.isnot(None)).count()
        )
        with_properties = (
            session.query(Garment).filter(Garment.properties_extracted_at.isnot(None)).count()
        )

        # Calculate coverage percentages
        clip_coverage = (with_clip_embeddings / total_garments * 100) if total_garments > 0 else 0
        openai_coverage = (
            (with_openai_embeddings / total_garments * 100) if total_garments > 0 else 0
        )
