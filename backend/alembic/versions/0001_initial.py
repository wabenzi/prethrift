...
def upgrade():
    ...
    op.create_table(
        'garment_attribute',
        sa.Column('id', sa.Integer, primary_key=True),
        sa.Column('garment_id', sa.Integer, sa.ForeignKey('garment.id', ondelete='CASCADE'), index=True),
        sa.Column('attribute_value_id', sa.Integer, sa.ForeignKey('attribute_value.id', ondelete='CASCADE'), index=True),
    ...
    op.create_table(
        ...
        sa.Column('processed', sa.Boolean, nullable=False, server_default=sa.text('false'), index=True),
        sa.Column('created_at', sa.DateTime, nullable=False),
    )
    ...
    op.create_table(
        'inventory_item',
        sa.Column('id', sa.Integer, primary_key=True),
        sa.Column('image_id', sa.Integer, sa.ForeignKey('inventory_image.id', ondelete='CASCADE'), index=True),
        sa.Column('garment_id', sa.Integer, sa.ForeignKey('garment.id', ondelete='SET NULL'), index=True),
        sa.Column('slot_index', sa.Integer, nullable=False),
        sa.Column('description', sa.String(length=2048)),
        sa.Column('description_embedding', sa.JSON),
        sa.Column('attributes_extracted', sa.Boolean, nullable=False, server_default=sa.text('false')),
